...
module.exports = async (req, res) => {
  ...
  try {
    switch (action) {
      ...
      case 'salesChart': return res.json({ ok: true, data: await getSalesChart(body.days || 14) });
...
// Data grafik penjualan: total omzet (transaksi sukses) per hari, N hari
// terakhir sampai hari ini. Dihitung dari tabel transactions langsung supaya
// selalu akurat (bukan cache), aman dipanggil tiap dashboard dibuka.
async function getSalesChart(days) {
  const n = Math.min(90, Math.max(7, parseInt(days, 10) || 14));
  ...
  // Siapkan bucket per-tanggal (YYYY-MM-DD, patokan waktu Asia/Jakarta) supaya
  // tetap ada titik 0 untuk hari yang tidak ada transaksinya (grafik tidak bolong).
  const buckets = {};
  const labels = [];
  for (let i = 0; i < n; i++) {
    const d = new Date(since); d.setDate(since.getDate() + i);
    const key = d.toLocaleDateString('sv-SE', { timeZone: 'Asia/Jakarta' }); // format YYYY-MM-DD
    buckets[key] = 0;
    labels.push(key);
  }
  (rows || []).forEach(r => {
    const key = new Date(r.created_at).toLocaleDateString('sv-SE', { timeZone: 'Asia/Jakarta' });
    if (key in buckets) buckets[key] += (r.total || 0);
  });
...
async function addStock(productId, linesText) {
  ...
  const { error: insertErr } = await supabase.from('stock_items').insert(rows);
  if (insertErr) throw new Error('Gagal menyimpan stok ke database: ' + insertErr.message);
  ...
  if (settings.auto_post_stock === 'true' && settings.channel_link) {
    const chatIdChannel = settings.channel_link.includes('t.me/') ? '@' + settings.channel_link.split('/').pop() : settings.channel_link;
    const r = await tg('sendMessage', {
      chat_id: chatIdChannel,
      text: `----➤ Stok <b>${escapeHtml(product.name)}</b> baru masuk +${lines.length} nih! Total ready <b>${totalReady}</b> akun. Yuk sikat sebelum ludes! 🎗️`,
      parse_mode: 'HTML'
    }).catch(e => ({ ok: false, description: String(e) }));
    channelPosted = !!r.ok;
    if (!r.ok) {
      channelError = r.description || 'unknown error';
      console.error('Gagal posting stok baru ke channel:', chatIdChannel, channelError);
    }
  }
  return { inserted: lines.length, totalReady, channelPosted, channelError };
}
